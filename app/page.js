...
export default function Page() {
  ...
  const navClass = (id) =>
    `hover:text-black ${
      active === id ? "text-black font-semibold underline underline-offset-4" : ""
    }`;
      ...
      <nav className="fixed top-0 left-0 w-full bg-white/80 backdrop-blur z-50 border-b">
        <div className="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center">
          ...
          <div className="hidden md:flex gap-6 text-sm font-medium">
            ...
            <button onClick={() => scrollTo("decision")} className={navClass("decision")}>
              Decision Paralysis
            </button>
            <button onClick={() => scrollTo("clarity")} className={navClass("clarity")}>
              Mental Clarity
            </button>
            <button onClick={() => scrollTo("discipline")} className={navClass("discipline")}>
              Discipline
            </button>
          </div>
